<?php 
    ...
    if ($_SESSION["loggedin"] == true && $_SESSION["phone"]){
        if(isset($_GET['post_id'])){
            ...
            $sql10 = "SELECT * FROM disliked WHERE user_id=$user_id AND post_id=$post_id";
            $r = $res = mysqli_query($conn,$sql5);
            if (mysqli_num_rows($r) < 1){
                if (mysqli_num_rows($result) === 1){
                    $row = mysqli_fetch_assoc($result);
                    $liked = (int)$row['disliked'] + 1;
                    $sql2 = "UPDATE post SET disliked=$liked WHERE id=$post_id";
                    $reslt = mysqli_query($conn,$sql2);
                    if ($reslt){
                        $sql1 = "INSERT INTO disliked (post_id,user_id) VALUES ($post_id,$user_id)";
                ...
                $row = mysqli_fetch_assoc($result);
                    ...
                    $reslt = mysqli_query($conn,$sql2);
                    if ($reslt){
                        $sql1 = "DELETE FROM disliked WHERE user_id=$user_id AND post_id=$post_id;";
                        $res = mysqli_query($conn,$sql1);
                        if ($res){
                            header("Location: detail.php?id=$post_id");
                        }
