  ...
  if ($_SESSION["loggedin"] == true && $_SESSION["phone"]){
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])){
            ...
            if (isset($_POST['title']) && isset($_POST['content']) && isset($_POST['code'])){
                ...
                $create_id = $info['id'];
                $sql = "INSERT INTO post (title, content, code, user_name, user_id, liked, disliked, down, comment_counter) VALUES ('$title', '$content', '$code', '$creator', $create_id, 0, $follow, $follow, $follow)";
                echo $sql;
                $query = mysqli_query($conn,$sql);
                if ($query){
                  $sql2 = "SELECT * FROM type WHERE phone = '$phone'";
                  $res = mysqli_query($conn,$sql2);
