        ...
        <div class="container">
            ...
            <?php 
                ...
                if ($_SESSION["loggedin"] == true && $_SESSION["phone"]){
                    ...
                    $sql = "SELECT username FROM users WHERE phone = '$phone'";
                    $res = mysqli_query($conn,$sql);
                    $info = $res->fetch_array()['username'];

                    echo "<h1 class='title is-1'>Hello, $info</h1>";
                    echo "<p class='title is-5'>Chia sẻ các câu chuyện của bạn một các chân thật nhất</p>";
                }
                else {
            ...
            <div class="columns">
                <div class="column">
                    <a href="create.php" class="btn btn-green w-100">Tạo bài viết</a>
                </div>
                <div class="column">
                  <a href="security.php" class="btn btn-outline-green w-100">Tạo bài viết ẩn danh</a>
                </div>
            </div>
            <div class="columns">
              <div class="column">
              <a href="security.php" class="btn btn-green w-100">Cùng phát triển website</a>
              </div>
            </div>
                    <?php 
                    ...
                    if(mysqli_num_rows($result) > 0) {
                        while($row = mysqli_fetch_array($result)) {
                            if ((int)$row['down'] < 1000){
                              ...
                              echo '<div class="card shadow-lg rounded">
                                ...
                                echo '  <p class="text" style="margin-bottom:15px;letter-spacing: 1px;">' .$row['content']. '</p>';
                                ...
                                echo    '<p class="title is-6 text-start" style="margin-bottom:15px;">Bình Luận: '.$row['comment_counter'].'</p>
                                        <form action="comment.php" method="GET" enctype="multipart/form-data"><div class="mb-3 form-group"><input type="number" name="post_id" id="post_id" value="'.$row['id'].'" hidden><input type="text" value="" class="form-control shadow" placeholder="Bình Luận" id="comment" name="comment"/></div><input name="submit" placeholder="Bình Luận" type="submit" class="btn btn-green shadow w-100" style="margin-top: 20px;" value="Bình Luận"/></form>
                                        <br><br>
                                      </div>
                                    </div>
                                    <br>';
                        ...
                        echo '<div class="card shadow-lg rounded">
                                <div class="card-body">
                                  <h1 class="text-center title is-3">Chưa có bài nào cả! Đăng đê bạn ơi</h1>
                                </div>
                              </div>  ';
                    }
                    ?>
